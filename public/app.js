...
botonAccesoProtegido,addEventListener('click', async () =>{
    ...
    if(!token){
        parrafoMensajeProtegido.textContent = 'Debes iniciar sesio para acceder al curso';
    ...
    try{
        ...
        if(res.ok){
            ...
            parrafoMensajeProtegido.textContent = data.error || ' UPS Acceso fallido al recurso';
            parrafoMensajeProtegido.style.color = 'red';
