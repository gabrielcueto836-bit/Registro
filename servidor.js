...
app.post("/registrar", async(req,res) =>{
    try{
        ...
        if (!username || !password ){
            return res.status(400).json({error:"el usuario y contraseña son requeridos"});
        ...
        if (error){
            ...
            return res.status(500).json({ error: "El registro fallo"});
...
app.get('/recurso-protegido', authenticateToken, (req, res) =>{
   ...
   res.status(200).json({
    ...
    data: 'Esto es solo para usuarios authenticos'
   });
